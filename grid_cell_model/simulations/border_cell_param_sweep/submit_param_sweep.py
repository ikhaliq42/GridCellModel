...
sim = DemoSimulation('../border_cells/simulation_grid.py', sim_label, dp)
...
p['bcMargin']               = 0 if o.bcMargin is None else o.bcMargin
...
for i in range(len(bcConnStds)):
    for j in range(len(bc_field_stds)):
        p['bcConnStd']    = bcConnStds[i]
        p['bc_field_std'] = bc_field_stds[j]
        sim._sim_label = "bcConnStd_" + str(int(bcConnStds[i])) + "_bc_field_std_" + str(int(bc_field_stds[j]))
