...
def bounding_box(self, shape)
    ...
    '''
    min_x = 0.0, max_x = 0.0
    miny_y = 0.0, max_y = 0.0
    for line in shape:
        for point in line:
	    min_x = min(min_x, point.x)
            max_x = max(max_x, point.x)
            min_y = min(min_y, point.y)
            max_x = max(max_y, point.y)

    return Position2D(min_x, max_y), Position2D(max_x, )
    
def bounding_box_dimensions(self, shape)
    '''
    Returns the length and height of a  bounding box enclosing a 2D shape
    defined by a set of lines.
    Shape is a list of Position2D tuples
    '''
...
def min_dist_to_line(self, pnt, l):
    '''
...
def scaleLine(self, originalLine, originalDim, newDim)
    '''
    ...
    '''
    x_ratio = newDim.x / originalDim.x
    y_ratio = newDim.y / originalDim.y
...
def squared_distance(self, start, end):
    '''
...
def substract2D(self, p1, p2):
    '''
    Vector subtraction of two Position2D objects
    i.e. subtract p2 from p1
    '''
    return Position2D(p2.x - p1.x, p2.y - p1.y) 

def add2D(self, p1, p2):
    '''
