...
from grid_cell_model.analysis import spikes
from grid_cell_model.analysis.image import Position2D
from grid_cell_model.analysis.image import remapTwistedTorus as twisted_torus_distance
...
def fit_gaussian_tt(sig_f, i):
    ...
    others = Position2D(X.flatten(), Y.flatten())
    a = Position2D(None, None)
...
class SingleBumpPopulation(spikes.TwistedTorusSpikes):
    ...
    def bump_position(self, tstart, tend, dt, win_len, full_err=True, winStep=None):
        ...
        tstart, tend, dt, win_len : float
            ...
            :meth:`~gridcells.analysis.spikes.PopulationSpikes.slidingFiringRate`.
        full_err : bool
